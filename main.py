# =========================================================
# RC87_RESPIRA_SENDER_CLEAN_SAFE
# Base: RC82 subida por Sergio.
# SOLO TOCA:
# - ritmo visual de preexperiencia: frases más lentas y con aire
# - sender pack visual limpio: fondo + vídeo 9:16 + reacción pequeña + botones
#
# NO toca:
# Stripe, webhook, Twilio/SMS, WhatsApp, video engine, DB,
# grabación, upload reacción, workers, cobros ni start-experience.
# =========================================================
...
print("🛟 RC87 RESPIRA SENDER CLEAN SAFE — MAIN COMPLETO + EL UMBRAL 🛟")

print("🛟 RC87 RESPIRA SENDER CLEAN SAFE — MAIN COMPLETO + ALMA YUL 🛟")
print("🛟 RC87 RESPIRA SENDER CLEAN SAFE — CARPETAS BLINDADAS 🛟")
print("🛟 RC87 RESPIRA SENDER CLEAN SAFE — /CREAR OK 🛟")
print("🛟 RC87 RESPIRA SENDER CLEAN SAFE — TODO METIDO PARA REVISAR 🛟")
print("🛟 RC87 RESPIRA SENDER CLEAN SAFE — YUL CUENTA LO QUE ESCRIBES 🛟")
print("🛟 RC87 RESPIRA SENDER CLEAN SAFE — FORMULARIO SIMPLE + MAGIA 🛟")
print("🛟 RC87 RESPIRA SENDER CLEAN SAFE — SOLO UN LUGAR 🛟")
print("🛟 RC87 RESPIRA SENDER CLEAN SAFE — FORMULARIO LIMPIO 🛟")
print("🛟 RC87 RESPIRA SENDER CLEAN SAFE — YUL NO BLOQUEA ETERNA 🛟")
print("🛟 RC87 RESPIRA SENDER CLEAN SAFE — SMS + MASTER V1 🛟")
import html
import json
import mimetypes
import os
...
# =========================================================
ETERNA_APP_VERSION = os.getenv("ETERNA_APP_VERSION", "RC87_RESPIRA_SENDER_CLEAN_SAFE").strip()
...
def admin_yul_version(token: str = ""):
    if ADMIN_TOKEN and token != ADMIN_TOKEN:
        raise HTTPException(status_code=403, detail="No autorizado")
    return {
        "version": "RC87_RESPIRA_SENDER_CLEAN_SAFE",
...
def admin_rc77_version(token: str = ""):
    if ADMIN_TOKEN and token != ADMIN_TOKEN:
        raise HTTPException(status_code=403, detail="No autorizado")
    return {"version":"RC87_RESPIRA_SENDER_CLEAN_SAFE","yul_uses_form_values":True,"post_consent_story_bridge":True,"auto_opens_after_camera_ready":True,"touches_critical_core":False}
...
@app.get("/admin/rc87-version")
def admin_rc87_version(token: str = ""):
    if ADMIN_TOKEN and token != ADMIN_TOKEN:
        raise HTTPException(status_code=403, detail="No autorizado")
    return {
        "version": "RC87_RESPIRA_SENDER_CLEAN_SAFE",
        "only_preexperience_and_sender_changed": True,
        "preexperience_breathes_more": True,
        "sender_pack_clean": True,
        "sender_pack_master_v1": "sender_pack_master_v1.png",
        "reaction_position": "top-right-small",
        "buttons": ["CREAR OTRA ETERNA", "COMPARTIR", "DESCARGAR"],
        "touches_critical_core": False,
    }
...
# =========================================================
# RC87 — PREEXPERIENCIA RESPIRA PATCH
# =========================================================

def rc87_preexperience_breath_patch(html_doc: str) -> str:
    """
    RC87: solo ritmo visual.
    Da más aire a las frases sin tocar experiencia, cámara ni circuito.
    """
    try:
        css = """
<style id="rc87-preexperience-breath">
.line,
.yul-line,
.yul-text,
.yul-story-line,
...
"""
        js = """
<script id="rc87-preexperience-breath-js">
(function(){
  const HOLD_EXTRA_MS = 1700;
  const BETWEEN_EXTRA_MS = 650;

  function sleep(ms){ return new Promise(r => setTimeout(r, ms)); }

  // Si existe showText como función global, lo hacemos respirar.
  if (typeof window.showText === "function" && !window.__RC87_SHOWTEXT_PATCHED__) {
    const originalShowText = window.showText;
    window.showText = async function(html, small, hold) {
      const baseHold = Number(hold || 2500);
      await originalShowText.call(this, html, small, baseHold + HOLD_EXTRA_MS);
      await sleep(BETWEEN_EXTRA_MS);
    };
    window.__RC87_SHOWTEXT_PATCHED__ = true;
  }

  function breatheLines(){
    try{
      const lines = Array.from(document.querySelectorAll(".line, .yul-line, .yul-text, .yul-story-line, .scene-text, .magic-text"));
      lines.forEach(function(el, i){
        el.style.setProperty("transition", "opacity 1.15s ease, transform 1.15s ease, filter 1.15s ease", "important");
        if(!el.dataset.rc87Breath){
          const currentDelay = parseFloat((getComputedStyle(el).animationDelay || "0").replace("s","")) || 0;
          el.style.animationDelay = (currentDelay + (i * 0.55)) + "s";
          el.style.animationDuration = "4.9s";
          el.dataset.rc87Breath = "1";
        }
      });
    }catch(e){}
  }
  ...
  if(document.readyState === "loading"){
    document.addEventListener("DOMContentLoaded", breatheLines);
  } else {
    breatheLines();
  }
  setTimeout(breatheLines, 400);
  setTimeout(breatheLines, 1200);
  setTimeout(breatheLines, 2500);

  try{
    if(window.RC79){
      if(window.RC79.minSceneMs) window.RC79.minSceneMs = Math.max(window.RC79.minSceneMs, 4200);
      if(window.RC79.maxSceneMs) window.RC79.maxSceneMs = Math.max(window.RC79.maxSceneMs, 11500);
    }
  }catch(e){}
})();
</script>
"""
        if "rc87-preexperience-breath" not in html_doc:
            html_doc = html_doc.replace("</head>", css + "</head>", 1) if "</head>" in html_doc else css + html_doc
        if "rc87-preexperience-breath-js" not in html_doc:
            html_doc = html_doc.replace("</body>", js + "</body>", 1) if "</body>" in html_doc else html_doc + js
    except Exception as e:
        print("[WARN] RC87 preexperience breath patch skipped:", e)
    return html_doc
...
def render_eterna_prologo_experience(recipient_token: str) -> HTMLResponse:
    ...
    html_doc = rc87_preexperience_breath_patch(html_doc)
...
def sender_pack(sender_token: str, view: str = ""):
    """
    RC87 — Sender Pack limpio.
    Fondo sender_pack_master_v1.png + vídeo 9:16 + reacción pequeña + botones.
    Sin etiquetas, sin controles decorativos, sin textos extra dentro del vídeo.
...
.veil{{position:absolute;inset:0;background:linear-gradient(180deg,rgba(0,0,0,.05) 0%,rgba(0,0,0,.12) 34%,rgba(0,0,0,.48) 100%);z-index:1;pointer-events:none}}

.header{{position:relative;z-index:3;text-align:center;padding-top:calc(env(safe-area-inset-top) + 12px)}}
.logo{{font-family:Georgia,"Times New Roman",serif;letter-spacing:.42em;color:#eec36a;font-size:clamp(16px,4.5vw,23px);text-shadow:0 0 22px rgba(255,200,93,.54)}}
.logo:after{{content:"♡";display:block;letter-spacing:0;margin-top:4px;font-size:15px;color:#ffd477}}
.title{{margin:9px auto 8px;width:90%;font-family:Georgia,"Times New Roman",serif;font-size:clamp(24px,7vw,38px);line-height:1.04;color:#fff5e8;text-shadow:0 0 22px rgba(255,255,255,.16)}}
.title span{{color:#f4c46c;text-shadow:0 0 30px rgba(255,199,92,.52)}}

.video-frame{{position:relative;z-index:4;width:min(74vw,372px);height:min(65svh,650px);margin:5px auto 12px;border-radius:34px;overflow:hidden;background:#000;border:1px solid rgba(255,215,136,.50);box-shadow:0 0 42px rgba(36,171,255,.28),0 0 58px rgba(255,191,83,.16),inset 0 0 24px rgba(255,255,255,.05)}}
.video-frame:before{{content:"";position:absolute;left:50%;top:9px;transform:translateX(-50%);width:68px;height:5px;border-radius:999px;background:rgba(255,236,190,.24);z-index:8;box-shadow:0 0 12px rgba(255,222,150,.20)}}
...
.reaction-video{{position:absolute;right:9px;top:52px;width:16.5%;min-width:52px;max-width:76px;aspect-ratio:9/16;border-radius:14px;overflow:hidden;background:#000;border:1.6px solid rgba(255,205,104,.96);box-shadow:0 0 0 1px rgba(255,245,207,.18),0 0 22px rgba(255,183,70,.58),inset 0 0 12px rgba(255,218,137,.18);z-index:9}}
...
@media(max-height:760px){{
  .header{{padding-top:calc(env(safe-area-inset-top) + 9px)}}
  .title{{font-size:clamp(21px,6.2vw,33px);margin:7px auto 5px}}
  .video-frame{{height:60svh;width:min(71vw,352px);border-radius:30px}}
  .btn{{min-height:47px}}
  .actions{{gap:8px;margin-bottom:calc(env(safe-area-inset-bottom) + 10px)}}
}}
@media(max-height:680px){{
  .logo:after{{display:none}}
  .title{{font-size:clamp(19px,5.6vw,30px);margin:5px auto}}
  .video-frame{{height:57svh;width:min(67vw,326px)}}
...
<div class="shell">
  ...
  <section class="video-frame" aria-label="Sender Pack ETERNA">
